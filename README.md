...
* **Informational Pages Module**: Complete `info` module providing About, Contact (with form + info components), Support, Terms of Service, Privacy Policy, and a custom 404 Not Found page — all built with shared `InfoPageHeader` and `InfoSection` components.
...
The codebase is built on a highly modular **Domain-Driven Architecture**:
  ...
  * `/src/app/` - The Next.js routing structure dynamically wrapping `(auth)`, `(onboarding)`, `(user)`, and `(admin)` zones. Protected pages use `Promise.all` for parallelized auth + data prefetch. Also contains a root `not-found.tsx` for global 404 handling.
  * `/src/modules/` - The beating heart of the system storing all explicitly modular UI/Server pairs (`user/profile`, `admin/dashboard`, `user/check-out`, `user/browse`, etc.). Complex views are decomposed into focused sub-components within their module's `ui/components/` directory. Includes the `info/` module for all informational pages (About, Contact, Support, Legal, Not Found) with shared reusable components (`InfoPageHeader`, `InfoSection`).
