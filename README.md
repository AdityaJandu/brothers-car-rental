...
To interact dynamically with the admin endpoints (`/dashboard`, `/add-car`, `/admin-booking`, `/audit-log`), you must alter your authenticated user session role manually to `admin` directly within the Postgres database via your active Supabase/Neon dashboard. All high-stakes admin modifications are safely recorded in the `audit_log` system and can be reviewed at `/audit-log`.
